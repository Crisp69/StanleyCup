...
if($include_check == "bXnqwa") {
	...
	if (!IsSet($id_news)) {echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0; URL=sc.php?id=news.php\">";}
	elseif (!IsSet($nick_comment) || !IsSet($text_comment)) {echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0; URL=sc.php?id=news.php&id_news=$id_news\">";}
    else {
...
$slovo = Split("[[:blank:]]+", $text_comment);		//rozdeleni textu na slova
	...
	$date_input = date("H:i:s d.m.Y", $time);
		...
		$file = "data/data/comments/$id_news.txt";
		...
		$write = StripSlashes(trim($date_input) ."|" . $nick_comment ."|". $text_comment ."|". $IP ."\n");
			if ((File_Exists($file)) && (Count(File($file))!==0)) {
				...
				echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0; URL=sc.php?id=news.php&id_news=$id_news\">";
	
    	}
	} 
	else {echo "<center>Calculate <b>Antispam</b> correctly!</center>"; include("news_comment_form.php");} 
    }
}
