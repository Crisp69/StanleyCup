	...
	while(!feof($f)) {
		...
		if (trim($tmp[0]) != "") {
			if ($team == trim($tmp[0])) {echo "<option SELECTED value=\"".trim($tmp[0]).";".trim($tmp[1]).";".trim($tmp[3])."\">&nbsp;".trim($tmp[1])."</option>";}
                ...
                else {echo "<option value=\"".trim($tmp[0]).";".trim($tmp[1]).";".trim($tmp[3])."\">&nbsp;".trim($tmp[1])."</option>";} 
			 }
		}
	}
	fclose($f);
...
$playoff_file = $_SERVER['DOCUMENT_ROOT']."/stanleycup/data/schedule/playoff".$current_season.".txt";
    ...
    if(file_exists($playoff_file)) {
        ...
        while (!feof($f)) {
            $tmp = explode("|", fgets($f, 2000));
    		if (trim($tmp[0]) != "") {
                if(($tmp[0] == "Stanley Cup Finals") && $tmp[10] != "?") {
    ...
    if($count_results ==0) {
        echo "<br /><br /><b>Stanley Cup Finals</b><br /><br /><table align=\"center\"><tr><td colspan=\"2\">";
    	...
    	if(file_exists($stadings_file)) {
        	...
        	while(!feof($f)) {
        		...
        		if (trim($tmp[0]) != "") {
        			...
        			teamlist($team, $conf="all", $pool = "q_full_a1");
        			}
        		}
        	}
        	...
        	while(!feof($f)) {
        		...
        		if (trim($tmp[0]) != "") {
        			...
        			teamlist($team, $conf="all", $pool = "q_full_b1");
        			}
        		}
        	}
        	...
        	$gm1 = "Stanley Cup Finals|".$tmp_q_full_a1[1]."|".$tmp_q_full_b1[1]."|".$tmp_q_full_a1[2]."|".$tmp_q_full_b1[2]."|".$tmp_qf1_date[0]."|".$tmp_qf1_date[1]."|".$tmp_qf1_date[2]."|".$tmp_q_full_a1[0]."|".$tmp_q_full_b1[0]."|?|?|1||1029\n";
        	$gm9 = "Stanley Cup Finals|".$tmp_q_full_b1[1]."|".$tmp_q_full_a1[1]."|".$tmp_q_full_b1[2]."|".$tmp_q_full_a1[2]."|".$tmp_qf2_date[0]."|".$tmp_qf2_date[1]."|".$tmp_qf2_date[2]."|".$tmp_q_full_b1[0]."|".$tmp_q_full_a1[0]."|?|?|1||1030\n";
        ...
        //	echo $gm16."<br />";
                ...
                $playoff_file_tmp = $_SERVER['DOCUMENT_ROOT']."/stanleycup/update/tmp/playoff".$current_season."scf.txt";
